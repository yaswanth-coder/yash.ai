...
<body><pre>${src.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</pre></body>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 sm:p-4">
      <div className="w-full max-w-7xl h-[94vh] sm:h-[92vh] bg-zinc-950 border border-zinc-800 rounded-xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-3.5 sm:px-5 py-2.5 sm:py-3 bg-zinc-900/90 border-b border-zinc-800 gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-300 min-w-0">
              <Code2 className="w-4 h-4 text-blue-400 shrink-0" />
              <span className="truncate">Canvas Studio</span>
              <span className="px-1.5 py-0.5 rounded-md bg-blue-500/15 text-blue-400 border border-blue-500/25 text-[10px] uppercase font-bold shrink-0">
                {language}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              onClick={handleRun}
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-md shadow-blue-600/20 transition-colors touch-target"
            >
              <Play className="w-3.5 h-3.5 fill-current" />
              <span className="hidden md:inline">Run Preview</span>
              <span className="md:hidden">Run</span>
            </button>

            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-semibold transition-colors touch-target"
              title="Copy Code"
            >
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors touch-target"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`flex flex-col ${activeTab === "editor" ? "flex-1" : "hidden lg:flex lg:w-1/2"} border-r border-zinc-800`}>
            <textarea
              ...
              className="flex-1 p-3.5 sm:p-5 bg-zinc-950 text-zinc-200 font-mono text-sm sm:text-xs resize-none focus:outline-none leading-relaxed custom-scrollbar"
              spellCheck={false}
