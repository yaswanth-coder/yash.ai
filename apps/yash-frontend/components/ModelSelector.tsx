...
import { ChevronDown, Cpu, Sparkles, Shield, Zap, Check, Plus } from "lucide-react";
import { ModelItem, fetchModels, CustomModelItem } from "@/services/providers";
...
const STATIC_MODELS: ModelItem[] = [
  ...
  { id: "xkiro:default", name: "xKiro AI", description: "High-performance inference via xKiro API" },
...
export default function ModelSelector({
  ...
  const loadModels = async () => {
    try {
      ...
      if (list && list.length > 0) {
        // Include standard providers as well as custom / xkiro models
  ...
  const handleCustomModelAdded = (newModel: CustomModelItem) => {
    const formatted: ModelItem = {
      id: newModel.model_id,
      name: `Custom: ${newModel.name}`,
      description: newModel.description || "Custom connected model",
      context_window: newModel.context_window,
    };
    setModels((prev) => [formatted, ...prev]);
    onSelectModel(newModel.model_id);
    setIsOpen(false);
  };
          ...
          >
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-0.5">
              {models.map((model) => {
                ...
                const isCustom = model.id.startsWith("custom:") || model.id.startsWith("xkiro:");
                return (
                  <button
                    key={model.id}
                    onClick={() => {
            ...
            <div className="pt-2 mt-1 border-t border-white/10">
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-400 hover:text-blue-300 text-xs font-semibold transition-all cursor-pointer"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>Add Model (Model ID & API Key)</span>
              </button>
            </div>
          </div>
        )}
